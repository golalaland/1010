...
function renderMessagesFromArray(messages) {
  ...
  // ✅ Check if at bottom BEFORE appending new messages
  const wasAtBottom = refs.messagesEl.scrollHeight - refs.messagesEl.scrollTop <= refs.messagesEl.clientHeight + 50; // 50px threshold
  ...
  if (wasAtBottom) {
    if (!scrollPending) {
      scrollPending = true;
      requestAnimationFrame(() => {
        refs.messagesEl.scrollTop = refs.messagesEl.scrollHeight; // Instant scroll for immediacy
        scrollPending = false;
      });
    }
  }
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  // ✅ Initialize the scroll-to-bottom button and listener
            ...
            if (index === stages.length - 1) {
              setTimeout(() => {
                modalContent.innerHTML = `
                  ...
                  <p style="margin-top:6px;padding:10px 18px;border:none;border-radius:8px;font-weight:600;background:linear-gradient(90deg,#ff0099,#ff6600);color:#fff;cursor:pointer;">Send Message</button>
                `;
...
function showSocialRedirectModal(modalContent, host) {
  ...
  if (socialUrl) {
    modalContent.innerHTML = `
      ...
      <p style="margin-bottom:16px;">${hostName} isn’t meeting new people new people via WhatsApp yet.</p>
...
function showHighlightsModal(videos) {
  ...
  toggleBtn.onmouseenter = () => {
    toggleBtn.style.background = "linear-gradient(135deg, #ff006e, #ff6600)";
