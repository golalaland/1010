...
function setGreeting() {
  ...
  const name = capitalizeFirstLetter(currentUser.chatId);
...
// Run on host panel open
document.getElementById("hostSettingsBtn")?.addEventListener("click", setGreeting);

/* ========== HIGHLIGHTS BUTTON ========== */
document.getElementById("highlightsBtn")?.addEventListener("click", async () => {
  if (!currentUser?.uid) {
    showStarPopup("Login required to view highlights");
    return;
  }

  try {
    const q = query(collection(db, "highlightVideos"), orderBy("createdAt", "desc"));
    const snapshot = await getDocs(q);

    if (snapshot.empty) {
      showStarPopup("No highlights available yet");
      return;
    }
    ...
    const videos = snapshot.docs.map(doc => {
      ...
      return {
        id: doc.id,
        ...d,
        uploaderName: d.uploaderName || d.chatId || "Anonymous",
    ...
    error("Failed to load highlights:", err);
    showStarPopup("Error loading highlights");
  }
});

/* ========== HIGHLIGHTS MODAL — FULLY REWRITTEN & PERFECT ========== */
function showHighlightsModal(videos) {
  ...
  Object.assign(modal.style, {
    ...
    backdropFilter: "blur(12px)", zIndex: 999999, overflowY: "auto",
    padding: "20px", boxSizing: "border-box", color: "#fff", fontFamily: "system-ui,sans-serif"
  });

  // Sticky Header
  const header = document.createElement("div");
  header.innerHTML = `
    <div style="max-width:640px;margin:0 auto 20px;background:rgba(255,255,255,0.08);padding:16px;border-radius:12px;text-align:center;">
      <p style="margin:0;font-size:14px;opacity:0.9;">
        Exclusive creator moments — unlock with ⭐ to support them!
      </p>
    </div>
  `;
  ...
  controls.style.cssText = "position:sticky;top:70px;z-index:1001;display:flex;gap:12px;justify-content:center;margin-bottom:20px;flex-wrap:wrap;";
  
  const searchWrapper = document.createElement("div");
  searchWrapper.innerHTML = `
    <div style="display:flex;align-items:center;background:rgba(255,255,255,0.1);border-radius:30px;padding:8px 16px;backdrop-filter:blur(8px);">
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#999" stroke-width="2"><path d="M15 15L21 21M10 17C6.13401 17 3 13.866 3 10C3 6.13401 6.13401 3 10 3C13.866 3 17 6.13401 17 10C17 13.866 13.866 17 10 17Z"/></svg>
      <input id="highlightSearch" placeholder="Search creator..." style="background:none;border:none;outline:none;color:#fff;margin-left:8px;width:180px;">
    </div>
  `;

  const toggleBtn = document.createElement("button");
  toggleBtn.textContent = "Show Unlocked Only";
  toggleBtn.style.cssText = "padding:8px 16px;background:#333;border:none;border-radius:30px;color:#fff;font-size:13px;cursor:pointer;";
  
  controls.append(searchWrapper, toggleBtn);
  modal.appendChild(controls);

  // Close Button (DOPE X)
  const closeX = document.createElement("div");
  closeX.innerHTML = `<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#ff006e" stroke-width="3"><path d="M18 6L6 18M6 6L18 18"/></svg>`;
  Object.assign(closeX.style, {
    position: "fixed", top: "20px", right: "20px", cursor: "pointer", zIndex: 1002,
    filter: "drop-shadow(0 0 8px #ff006e)", transition: "transform 0.2s"
  });
  closeX.onmouseenter = () => closeX.style.transform = "rotate(90deg) scale(1.2)";
  closeX.onmouseleave = () => closeX.style.transform = "rotate(0) scale(1)";
  closeX.onclick = () => modal.remove();
  modal.appendChild(closeX);

  // Content Grid
  const grid = document.createElement("div");
  grid.style.cssText = "display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:20px;max-width:1200px;margin:0 auto;";
  ...
  function render() {
    grid.innerHTML = "";
    const filtered = videos.filter(v => !showUnlockedOnly || unlockedVideos.includes(v.id));
    ...
    filtered.forEach(video => {
      ...
      card.style.cssText = `
        ...
        transition:all 0.3s; box-shadow:0 4px 20px rgba(255,0,110,0.15);
      `;
      ...
      videoEl.loop = videoEl.muted = !isUnlocked;
      videoEl.playsInline = videoEl.preload = isUnlocked ? "auto" : "metadata";
      videoEl.style.cssText = `width:100%;height:340px;object-fit:cover;${!isUnlocked ? 'filter:blur(8px);' : ''}`;

      if (!isUnlocked) {
        card.onmouseenter = () => videoEl.play().catch(() => {});
        card.onmouseleave = () => { videoEl.pause(); videoEl.currentTime = 0; };
      } else {
        videoEl.autoplay = true;
      }

      const overlay = !isUnlocked ? `
        <div style="position:absolute;inset:0;background:linear-gradient(135deg,rgba(0,0,0,0.7),rgba(255,0,110,0.3));display:flex;align-items:center;justify-content:center;">
          <svg width="60" height="60" viewBox="0 0 24 24" fill="#ff006e"><path d="M12 2C9.2 2 7 4.2 7 7V11H6C4.9 11 4 11.9 4 13V20C4 21.1 4.9 22 6 22H18C19.1 22 20 21.1 20 20V13C20 11.9 19.1 11 18 11H17V7C17 4.2 14.8 2 12 2ZM12 4C13.7 4 15 5.3 15 7V11H9V7C9 5.3 10.3 4 12 4Z"/></svg>
        </div>
      ` : "";

      const info = `
        <div style="padding:14px;background:#111;">
          <div style="font-weight:700;font-size:15px;margin-bottom:4px;">${video.title || "Untitled"}</div>
          <div style="color:#ff006e;font-size:13px;margin-bottom:8px;">By ${video.uploaderName}</div>
          <button class="unlock-btn" ${isUnlocked ? 'disabled style="background:#333"' : ''}>
            ${isUnlocked ? "Unlocked" : `Unlock ${video.highlightVideoPrice} ⭐`}
          </button>
        </div>
      `;

      card.innerHTML = `<div style="position:relative;">${videoEl.outerHTML}${overlay}</div>${info}`;
      card.onclick = (e) => {
        e.stopPropagation();
        if (isUnlocked) playFullVideo(video);
        else showUnlockConfirm(video, () => showHighlightsModal(videos));
      };
  ...
  // Search
  searchWrapper.querySelector("input").addEventListener("input", e => {
    const term = e.target.value.toLowerCase();
    grid.querySelectorAll(".videoCard").forEach(card => {
      const matches = card.dataset.uploader.toLowerCase().includes(term) || 
  ...
  toggleBtn.onclick = () => {
    ...
    toggleBtn.textContent = showUnlockedOnly ? "Show All" : "Show Unlocked Only";
    render();
  };
...
/* ========== UNLOCK CONFIRM & TRANSACTION ========== */
async function handleUnlockVideo(video) {
  if (!currentUser?.uid) return showStarPopup("Login required");

  const price = Number(video.highlightVideoPrice) || 100;
  if (currentUser.stars < price) return showStarPopup("Not enough stars");

  try {
    const senderRef = doc(db, "users", currentUser.uid);
    const receiverRef = doc(db, "users", video.uploaderId);
    ...
    await runTransaction(db, async tx => {
      ...
      if (!senderSnap.exists()) throw "User not found";
      if (senderSnap.data().stars < price) throw "Insufficient stars";
      ...
      if (receiverRef) {
        tx.update(receiverRef, { stars: increment(price) });
      }
    ...
    showStarPopup(`Unlocked: ${video.title}!`);
  } catch (err) {
    showStarPopup("Unlock failed: " + (err.message || err));
  }
}

function showUnlockConfirm(video, callback) {
  document.getElementById("unlockConfirmModal")?.remove();

  const modal = document.createElement("div");
  modal.id = "unlockConfirmModal";
  modal.style.cssText = "position:fixed;inset:0;background:rgba(0,0,0,0.95);backdrop-filter:blur(10px);display:flex;align-items:center;justify-content:center;z-index:1000001;";
  modal.innerHTML = `
    <div style="background:#111;padding:24px;border-radius:16px;text-align:center;max-width:320px;width:90%;box-shadow:0 0 30px rgba(255,0,110,0.3);">
      <h3>Unlock "${video.title}"?</h3>
      <p>Cost: <strong>${video.highlightVideoPrice} ⭐</strong></p>
      <div style="display:flex;gap:12px;justify-content:center;margin-top:16px;">
        <button onclick="this.closest('#unlockConfirmModal').remove()" style="padding:10px 20px;background:#333;border:none;border-radius:8px;color:#fff;">Cancel</button>
        <button id="confirmUnlockBtn" style="padding:10px 20px;background:linear-gradient(90deg,#ff006e,#ff4500);border:none;border-radius:8px;color:#fff;font-weight:600;">Unlock</button>
      </div>
    </div>
  `;

  modal.querySelector("#confirmUnlockBtn").onclick = async () => {
    modal.remove();
...
function playFullVideo(video) {
  const modal = document.createElement("div");
  modal.style.cssText = "position:fixed;inset:0;background:#000;display:flex;align-items:center;justify-content:center;z-index:1000002;";
  
  const vid = document.createElement("video");
  vid.src = video.highlightVideo;
  vid.controls = vid.autoplay = true;
  vid.style.cssText = "max-width:95%;max-height:95%;border-radius:12px;box-shadow:0 0 40px rgba(0,0,0,0.8);";
...
/* ========== NOTIFY NEW CONTENT (GLOW PULSE) ========== */
function notifyNewContent(selector) {
  const el = document.querySelector(selector);
  if (!el) return;
  el.classList.remove("glow-pulse");
  void el.offsetWidth;
  el.classList.add("glow-pulse");
  setTimeout(() => el.classList.remove("glow-pulse"), 8000);
}

// Example usage
notifyNewContent("#highlightsBtn");
notifyNewContent("#topBallersBtn");

/* ========== $STRZ AIRDROP MODAL (CLEAN & WORKING) ========== */
function showStrzAirdropModal() {
  document.getElementById("strzAirdropModal")?.remove();

  const modal = document.createElement("div");
  modal.id = "strzAirdropModal";
  modal.style.cssText = "position:fixed;inset:0;background:rgba(0,0,0,0.96);backdrop-filter:blur(16px);display:flex;align-items:center;justify-content:center;z-index:999999;";
  
  modal.innerHTML = `
    <div style="background:linear-gradient(135deg,#0a001a,#1a0033);padding:30px;border-radius:20px;max-width:360px;width:90%;text-align:center;position:relative;overflow:hidden;border:2px solid #333;">
      <div style="position:absolute;inset:-4px;border-radius:22px;background:conic-gradient(#ffd700,#ff00ff,#00ffff,#ffd700);animation:spin 6s linear infinite;opacity:0.7;z-index:-1;"></div>
      <button onclick="this.closest('#strzAirdropModal').remove()" style="position:absolute;top:12px;right:16px;background:none;border:none;color:#fff;font-size:32px;cursor:pointer;">×</button>
      <h2 style="background:linear-gradient(90deg,#ffd700,#ff00ff);-webkit-background-clip:text;-webkit-text-fill-color:transparent;font-size:22px;margin:0 0 20px;">$STRZ AIRDROP</h2>
      <div id="countdown" style="font-size:56px;font-weight:900;margin:20px 0;background:linear-gradient(90deg,#00ffff,#ff00ff);-webkit-background-clip:text;-webkit-text-fill-color:transparent;">10:00</div>
      <button id="claimBtn" style="width:90%;padding:16px;border:none;border-radius:50px;background:linear-gradient(90deg,#ffd700,#ff006e);color:#000;font-weight:900;font-size:19px;cursor:pointer;box-shadow:0 10px 30px rgba(255,215,0,0.4);">CLAIM 500 ⭐ FREE</button>
      <p style="margin-top:16px;font-size:13px;opacity:0.8;">Every 30 mins → Random winner gets <b>1000 ⭐</b></p>
    </div>
  `;
  ...
  const btn = modal.querySelector("#claimBtn");
  ...
  const timer = setInterval(() => {
    ...
    if (seconds <= 0) {
      ...
      btn.style.background = "linear-gradient(90deg,#ff0000,#ffff00)";
      notifyNewContent("#topBallersBtn");
    }
  }, 1000);

  btn.onclick = () => {
    if (seconds > 0) return showStarPopup("Wait for countdown!");
    showStarPopup("500 ⭐ CLAIMED! Keep ballin'");
    modal.remove();
  };
...
// Attach to button
document.getElementById("topBallersBtn")?.addEventListener("click", showStrzAirdropModal);
